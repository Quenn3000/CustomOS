...
class PeriphericalComponnentInterconnectController {
	...
	private:
		...
		bool deviceHasFunction(uint8_t bus, uint16_t device);

		uint32_t get_id(uint8_t bus, uint16_t device, uint8_t function, uint32_t registerOffset);

		PeriphericalComponnentInterconnectedDeviceDescriptor getDeviceDescriptor(uint8_t bus, uint16_t device, uint8_t function);
